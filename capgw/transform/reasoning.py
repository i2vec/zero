...
Vendored from polar.gateway.transform.reasoning (unmodified).

An upstream model's `--reasoning-parser` (split mode: `qwen3`, `minimax`,
`deepseek-r1`, etc.) splits the model's chain-of-thought into the assistant
message's `reasoning_content` field. This module helps each transform convert
that field to / from the API-specific reasoning shape:
