...
Vendored from polar.gateway.transform.anthropic (import prefixes rewritten).
...
from capgw.transform.images import (
    anthropic_content_to_openai_chat,
    openai_chat_content_to_anthropic_blocks,
)
from capgw.transform.reasoning import (
...
# rendered prompt tokens drift every turn. Strip the line before forwarding.
...
class AnthropicTransformer(BaseTransformer):
    ...
    def transform_request(self, body: dict[str, Any]) -> dict[str, Any]:
        ...
        # the upstream reject with "tool_choice only allowed when tools specified".
        if "tools" in body:
    ...
    def _transform_tools_to_openai(self, tools: list[dict]) -> list[dict]:
        ...
        for tool in tools:
            ...
            # explicit `type` and have no `input_schema`. Drop rather than
            # forwarding a stub function tool.
